...
int main()
{
    ...
    unsigned int shader = lx_shader_create((lx_shader_properties){
        .vertex_file = "shaders/default.vert",
        .fragment_file = "shaders/default.frag"
    });
    ...
    lx_mat4 projection = lx_mat4_perspective(45.0f, 1920.0f/1080.0f, 0.1f, 100.0f);
    ...
    while (lx_window_is_alive(window))
    {
        ...
        model = lx_mat4_rotate(model, (lx_vec3){ 0.0f, 1.0f, 0.0f }, 45.0f * lx_window_get_delta_time(window));
        glUniformMatrix4fv(glGetUniformLocation(shader, "model"), 1, GL_FALSE, model.m);
